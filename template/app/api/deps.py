...
async def get_current_user(
    ...
    try:
        ...
        # Attach the raw_token so downstream role checks can use
        # it for UserInfo calls
...
def require_role(required_role: str) -> Callable:
    ...
    async def role_checker(
        ...
        if required_role not in user_roles:
            logger.warning(
                f"🚨 403: User {current_user.sub} attempted action "
                f"requiring '{required_role}'. "
                f"Current roles: {user_roles}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=(
                    f"Role '{required_role}' is required to "
                    "perform this action."
                ),
            )
